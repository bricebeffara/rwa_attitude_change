# File name: brms_models_xp01.R
# Online archive: gitlab
# Authors: Brice Beffara & Amélie Bret 
# Sat Aug 04 15:58:34 2018 ------------------------------
...
rrwa <- round(max ( base_df$RWAscore) - min ( base_df$RWAscore))
...
# model
base_resp <- brm(response ~ usvalence  * RWAscore + (1|ppt) + (1|stim1),
                   data = base_df, 
...
# model_gen_xp01 <- summary(base_resp)$fixed[,col2keep]
# model_gen_xp01 <- round(model_gen_xp01, 3)

# arrange output
model_gen_xp01 <- tidy_stan(base_resp,
                            typical = "mean",
                            prob = .95)
...
# test with rope
equi_gen_xp01_rwa <- equi_test(base_resp, rope = roperwaN)
equi_gen_xp01_rwa <- equi_gen_xp01_rwa[c(10,11),]
equi_gen_xp01_rwa$ROPE <- roperwaC

equi_gen_xp01_0.5 <- equi_test(base_resp, rope = c(-0.5, 0.5))
...
# See 02b_decint2_brms_models_xp01.R for other models 