...
# We take the lowest and highest integers to compute a global maximum range of RWA

# low RWA
lrwa <- ceiling(min ( spreading$RWAscore))

# high RWA
hrwa <- floor(max ( spreading$RWAscore))

# RWA range

rrwa <- hrwa - lrwa

# ROPE = 1 (minimum change on rating scale) / 5 (global RWA range) / 2 (e.g. see Kruschke, 2018 p.6)
# (here we devide or smallest possible effect by two in order to take variations into account,
# e.g. RWA range above limits and avoid to be too conservative just above/below lower/upperlimits)

roperwaN <- c( -1/rrwa/2, 1/rrwa/2)
roperwaC <- paste( as.character(-1/rrwa/2), ", ", as.character(1/rrwa/2), sep = "")
