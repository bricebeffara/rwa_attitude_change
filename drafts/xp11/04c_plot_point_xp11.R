...
warn_df$Avertissement <- ifelse (warn_df$warn == -0.5, "Sans", "Avec")

# negative plot -----------------------------------------------------------

warn_neg <- subset(warn_df, usvalence==-0.5)

data_plot_neg = warn_neg %>%
  ggplot(aes(x = as.character(response), y = RWAscore,
             fill = Avertissement, color = Avertissement)) +
  ...
  labs(subtitle="Valence négative") +
...
warn_pos <- subset(warn_df, usvalence==0.5)

data_plot_pos = warn_pos %>%
  ggplot(aes(x = as.character(response), y = RWAscore,
             fill = Avertissement, color = Avertissement)) +
  #geom_boxplot(position = position_dodge(width = -1.4), alpha = 1, width = 0.1, outlier.shape = NA) +
  geom_point(position = position_jitterdodge(jitter.width = 0.2, jitter.height = 0.05, 
  ...
  coord_cartesian(ylim=c(1,9)) +
  scale_y_continuous(breaks = scales::pretty_breaks(n = 10), expand=c(0.01,0)) +
  labs(subtitle="Valence positive") +
  theme_ipsum_rc(base_size = 13,
                 subtitle_size = 20,
                 axis_title_size = 15) +
  guides(fill = guide_legend(override.aes = list(linetype = 0)),
...
# save plot
ggsave("plots/data_plot_all_xp11_french.pdf", width = 50, height = 15, units = "cm")
