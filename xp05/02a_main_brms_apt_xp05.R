# File name: 02a_main_brms_apt_xp05.R
# Online archive: gitlab
# Authors: Brice Beffara & Amélie Bret 
# Wed Aug 15 18:16:47 2018 ------------------------------
...
priors <- c(
  prior(normal(750, 200), class = Intercept, coef = ""),
...
rrwa <- round(max ( apt_df$RWAscore) - min ( apt_df$RWAscore))
...
# model_gen_xp05 <- summary(apt_resp)$fixed[,col2keep]
...
# test with rope
equi_gen_xp05_rwa <- equi_test(apt_resp, rope = roperwaN)
equi_gen_xp05_rwa <- equi_gen_xp05_rwa[c(4,6:8),]
equi_gen_xp05_rwa$ROPE <- roperwaC

equi_gen_xp05_0.5 <- equi_test(apt_resp, rope = c(-8, 8))
equi_gen_xp05_0.5 <- equi_gen_xp05_0.5[c(2,3,5),]
equi_gen_xp05_0.5$ROPE <- "-8, 8"
