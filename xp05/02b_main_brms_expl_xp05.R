# File name: brms_models_xp05.R
# Online archive: gitlab
# Authors: Brice Beffara & Amélie Bret 
# Thu Aug 16 14:29:24 2018 ------------------------------
# Contact: brice.beffara@slowpen.science amelie.bret@univ-grenoble-alpes.fr http://slowpen.science
#
# This R script was used to build and compute brms models
# corresponding to the 5th experiment of Amelie Bret's doctoral work
#
...
# expliing packages needed (and installing if necessary) for this part
...
rrwa <- round(max ( expli_df$RWAscore) - min ( expli_df$RWAscore))
...
# model
expli_resp <- brm(response ~ usvalence * order  * RWAscore + (1|ppt) + (1|stim1),
                   data = expli_df, 
...
# model_gen_xp05 <- summary(expli_resp)$fixed[,col2keep]
# model_gen_xp05 <- round(model_gen_xp05, 3)

# arrange output
expli_gen_xp05 <- tidy_stan(expli_resp,
                            typical = "mean",
                            prob = .95)


# export output
png("tables/brms/expli_gen_xp05.png", height=480, width=720)
p<-tableGrob(expli_gen_xp05)
grid.arrange(p)
dev.off()

# test with rope
equi_gen_expli__xp05_rwa <- equi_test(expli_resp, rope = roperwaN)
equi_gen_expli__xp05_rwa <- equi_gen_expli__xp05_rwa[c(11,13:15),]
equi_gen_expli__xp05_rwa$ROPE <- roperwaC

equi_gen_expli__xp05_0.5 <- equi_test(expli_resp, rope = c(-0.5, 0.5))
equi_gen_expli__xp05_0.5 <- equi_gen_expli__xp05_0.5[c(9,10,12),]
equi_gen_expli__xp05_0.5$ROPE <- "-0.5, 0.5"

equi_gen_expli__xp05 <- rbind(equi_gen_expli__xp05_rwa, equi_gen_expli__xp05_0.5)
equi_gen_expli__xp05[,c(3:5)] <- round(equi_gen_expli__xp05[,c(3:5)], 2)
ordrow <- c("5","6","1","7","2","3","4")
equi_gen_expli__xp05 <- equi_gen_expli__xp05[ordrow,]

# export test
png("tables/rope/equi_gen_expli__xp05.png", height=480, width=720)
p<-tableGrob(equi_gen_expli__xp05)
grid.arrange(p)
dev.off()

# See 02c_decint2_brms_expl_xp05.R for other models 