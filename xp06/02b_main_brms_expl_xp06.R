...
# and order : directcit first (0.5) vs. implicit first (-0.5)
#
...
# directing packages needed (and installing if necessary) for this part
...
rrwa <- round(max ( direct_df$RWAscore) - min ( direct_df$RWAscore))
...
# model
direct_resp <- brm(response ~ usvalencedir * order * RWAscore + (1|ppt) + (1|stim1),
                   data = direct_df, 
...
# model_gen_xp06 <- summary(direct_resp)$fixed[,col2keep]
# model_gen_xp06 <- round(model_gen_xp06, 3)

# arrange output
direct_gen_xp06 <- tidy_stan(direct_resp,
                            typical = "mean",
                            prob = .95)


# export output
png("tables/brms/direct_gen_xp06.png", height=480, width=720)
p<-tableGrob(direct_gen_xp06)
grid.arrange(p)
dev.off()

# test with rope
equi_gen_direct__xp06_rwa <- equi_test(direct_resp, rope = roperwaN)
equi_gen_direct__xp06_rwa <- equi_gen_direct__xp06_rwa[c(11,13:15),]
equi_gen_direct__xp06_rwa$ROPE <- roperwaC

equi_gen_direct__xp06_0.5 <- equi_test(direct_resp, rope = c(-0.5, 0.5))
equi_gen_direct__xp06_0.5 <- equi_gen_direct__xp06_0.5[c(9,10,12),]
equi_gen_direct__xp06_0.5$ROPE <- "-0.5, 0.5"

equi_gen_direct__xp06 <- rbind(equi_gen_direct__xp06_rwa, equi_gen_direct__xp06_0.5)
equi_gen_direct__xp06[,c(3:5)] <- round(equi_gen_direct__xp06[,c(3:5)], 2)
ordrow <- c("5","6","1","7","2","3","4")
equi_gen_direct__xp06 <- equi_gen_direct__xp06[ordrow,]

# export test
png("tables/rope/equi_gen_direct__xp06.png", height=480, width=720)
p<-tableGrob(equi_gen_direct__xp06)
grid.arrange(p)
dev.off()
