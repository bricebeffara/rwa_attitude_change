...
attr(marg_lvone$`RWAscore:usvalence`, "spaghetti") <- attr(marg_lvone$`RWAscore:usvalence`, "spaghetti")[grepl(0.5, attr(marg_lvone$`RWAscore:usvalence`, "spaghetti")$sample__),]

attr(marg_lvtwo$`RWAscore:usvalence`, "spaghetti") <- attr(marg_lvtwo$`RWAscore:usvalence`, "spaghetti")[grepl(0.5, attr(marg_lvtwo$`RWAscore:usvalence`, "spaghetti")$sample__),]

attr(marg_lvone$`RWAscore:usvalence`, "spaghetti")$usvalence <- ifelse (attr(marg_lvone$`RWAscore:usvalence`, "spaghetti")$usvalence == -0.5, "Négative", "Positive")

attr(marg_lvtwo$`RWAscore:usvalence`, "spaghetti")$usvalence <- ifelse (attr(marg_lvtwo$`RWAscore:usvalence`, "spaghetti")$usvalence == -0.5, "Négative", "Positive")

# !!orginally!! conditioned stimuli with !!positive!! valence
marg_plot_lvone = plot(marg_lvone, plot = FALSE, mean = F)[[1]] + # here his where we specify the marginal effects of interest
  ...
  labs(colour="Valence",
       subtitle="Conditionnnement de niveau 1") + # here his where we mention the marginal effects of interest
...
# !!orginally!! conditioned stimuli with !!positive!! valence
marg_plot_lvtwo = plot(marg_lvtwo, plot = FALSE, mean = F)[[1]] + # here his where we specify the marginal effects of interest
  scale_y_continuous(name="Évaluations", breaks = c(1, 2, 3, 4, 5, 6, 7, 8, 9), expand = c(0,0)) +
  coord_cartesian(ylim=c(1,9)) +
  #scale_colour_hue(labels = c("Level 1", "Level 2")) +
  scale_x_continuous(name="RWA", breaks = scales::pretty_breaks(n = 10), expand=c(0.01,0)) +
  labs(colour="Valence",
       subtitle="Conditionnnement de niveau 2") + # here his where we mention the marginal effects of interest
  theme_ipsum_rc(base_size = 13,
                 subtitle_size = 20,
                 axis_title_size = 15)
...
# Combine plot
marg_spag_all <- ggarrange(marg_plot_lvone,
                           marg_plot_lvtwo,
                      ncol = 2, nrow = 1)
...
# marg_spag_all
# ggsave("plots/marg_spag_xp10_french.jpg", width = 50, height = 15, units = "cm")
# ggsave("plots/marg_spag_xp10_french.pdf", width = 50, height = 15, units = "cm")
# ggsave("plots/marg_spag_xp10_french.tex", width = 50, height = 15, units = "cm")
