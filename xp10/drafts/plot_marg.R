...
marg_on <- marginal_effects(spread_resp, effects = "RWAscore", ordinal = TRUE, 
                             conditions = cond_on, method = c("predict"), # here his where we specify the combination
                             re_formula = NULL)$RWAscore

# !!orginally!! conditioned stimuli with !!positive!! valence
marg_op <- marginal_effects(spread_resp, effects = "RWAscore", ordinal = TRUE, 
                             conditions = cond_op, method = c("predict"), # here his where we specify the combination
                             re_formula = NULL)$RWAscore

# !!spread!! conditionned stimuli with !!negative!! valence
marg_sn <- marginal_effects(spread_resp, effects = "RWAscore", ordinal = TRUE, 
                             conditions = cond_sn, method = c("predict"), # here his where we specify the combination
                             re_formula = NULL)$RWAscore

# !!spread!! conditionned stimuli with !!positive!! valence
marg_sp <- marginal_effects(spread_resp, effects = "RWAscore", ordinal = TRUE, 
                             conditions = cond_sp, method = c("predict"), # here his where we specify the combination
                             re_formula = NULL)$RWAscore
...
# save plot
ggsave("plots/marg_all_xp10.pdf", width = 50, height = 30, units = "cm")
