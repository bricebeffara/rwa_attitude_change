...
spreading$Conditionnement <- ifelse (spreading$spreading == -0.5, "Niveau 1", "Niveau 2")
...
data_plot_neg = spreading_neg %>%
  ggplot(aes(x = as.character(response), y = RWAscore,
             fill = Conditionnement, color = Conditionnement)) +
...
# save plot
ggsave("plots/data_plot_all_xp10_french.pdf", width = 50, height = 15, units = "cm")
