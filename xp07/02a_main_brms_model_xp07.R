# File name: brms_models_xp07.R
# Online archive: gitlab
# Authors: Brice Beffara & Amélie Bret 
# Fri Jul 06 22:01:02 2018 ------------------------------
# Contact: brice.beffara@slowpen.science amelie.bret@univ-grenoble-alpes.fr http://slowpen.science
#
# This R script was used to build and compute brms models
# corresponding to the 07th experiment of Amelie Bret's doctoral work
#
...
rrwa <- round(max ( double_df$RWAscore) - min ( double_df$RWAscore))
...
# model
double_resp <- brm(response ~ usvalence * counter2 * RWAscore + (1|ppt) + (1|stim1),
                   data = double_df, 
...
# model_gen_xp07 <- summary(double_resp)$fixed[,col2keep]
# model_gen_xp07 <- round(model_gen_xp07, 3)

# arrange output
model_gen_xp07 <- tidy_stan(double_resp,
                            typical = "mean",
                            prob = .95)
...
# test with rope
equi_gen_xp07_rwa <- equi_test(double_resp, rope = roperwaN)
equi_gen_xp07_rwa <- equi_gen_xp07_rwa[c(11,13:15),]
equi_gen_xp07_rwa$ROPE <- roperwaC

equi_gen_xp07_0.5 <- equi_test(double_resp, rope = c(-0.5, 0.5))
equi_gen_xp07_0.5 <- equi_gen_xp07_0.5[c(9,10,12),]
...
# See 02b_decint2_brms_models_xp07.R for other models 