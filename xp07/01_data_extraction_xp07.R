###########
#### RWA & double counter-conditioning: Experiment 1
###########
...
df[subjectGroup %in% c("1", "3") & family %in% "M", usvalence := -0.5]
df[subjectGroup %in% c("1", "3") & family %in% "B", usvalence := 0.5]
df[subjectGroup %in% c("2", "4") & family %in% "M", usvalence := 0.5]
df[subjectGroup %in% c("2", "4") & family %in% "B", usvalence := -0.5]

## Recode experimental condition

# subjectGroup 1, 2: conditioning - counterconditioning - ratings                       = condition -0.5
# subjectGroup 3, 4: conditioning - counterconditioning - counterconditioning - ratings = condition 0.5

df[subjectGroup %in% c("1", "2"), counter2 := -0.5]
df[subjectGroup %in% c("3", "4"), counter2 := 0.5]
