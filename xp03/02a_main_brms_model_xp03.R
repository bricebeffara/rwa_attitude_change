...
# model
bloc_resp <- brm(response ~ usvalence2 * bloc  * RWAscore + (1|ppt) + (1|stim1),
                   data = bloc_df, 
...
# model_gen_xp03 <- summary(bloc_resp)$fixed[,col2keep]
# model_gen_xp03 <- round(model_gen_xp03, 3)

# arrange output
model_gen_xp03 <- tidy_stan(bloc_resp,
                            typical = "mean",
                            prob = .95)
...
# test with rope
equi_gen_xp03_rwa <- equi_test(bloc_resp, rope = roperwaN)
equi_gen_xp03_rwa <- equi_gen_xp03_rwa[c(11,13:15),]
equi_gen_xp03_rwa$ROPE <- roperwaC

equi_gen_xp03_0.5 <- equi_test(bloc_resp, rope = c(-0.5, 0.5))
equi_gen_xp03_0.5 <- equi_gen_xp03_0.5[c(9,10,12),]
...
# See 02b_decint2_brms_models_xp03.R for other models 