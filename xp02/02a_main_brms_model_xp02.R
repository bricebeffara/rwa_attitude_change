...
# model
social_resp <- brm(response ~ usvalence * social  * RWAscore + (1|ppt) + (1|stim1),
                   data = social_df, 
...
# model_gen_xp02 <- summary(social_resp)$fixed[,col2keep]
# model_gen_xp02 <- round(model_gen_xp02, 3)

# arrange output
model_gen_xp02 <- tidy_stan(social_resp,
                            typical = "mean",
                            prob = .95)
...
# test with rope
equi_gen_xp02_rwa <- equi_test(social_resp, rope = roperwaN)
equi_gen_xp02_rwa <- equi_gen_xp02_rwa[c(11,13:15),]
equi_gen_xp02_rwa$ROPE <- roperwaC

equi_gen_xp02_0.5 <- equi_test(social_resp, rope = c(-0.5, 0.5))
equi_gen_xp02_0.5 <- equi_gen_xp02_0.5[c(9,10,12),]
...
# See 02b_decint2_brms_models_xp02.R for other models 